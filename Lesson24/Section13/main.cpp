...
int main() {
  ...
  while (getline(in_file, line) && (i<=3)) {
    ...
    int64_t timestamp;
    if (sensor_type.compare("L") == 0) {  // laser measurement NOLINT(readability-string-compare)
      // read measurements
    ...
    } else if (sensor_type.compare("R") == 0) { // NOLINT(readability-string-compare)
  ...
  for (size_t k = 0; k < N; ++k) {
      cout << "Measurements = " << measurement_pack_list[k].raw_measurements_ << endl;
