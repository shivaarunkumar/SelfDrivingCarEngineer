...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions) {
     ...
     * Your solution here.
     */
     std::vector<string> successors = this->successor_states();
     vector<vector<Vehicle>> trajectories;
     vector<float> costs;

     for(auto sstate: successors){
          trajectories.push_back(generate_trajectory(sstate, predictions));
     }
     ...
     for(auto cost: costs){
         if(cost<=cost_min){
             cost_min = cost;
             final_trajectory = trajectories[i];
         }
         i++;
     }
