...
vector<HBF::maze_s> HBF::expand(HBF::maze_s &state, vector<int> &goal) {
    ...
    for(double delta_i = -35; delta_i < 40; delta_i+=5) {
        ...
        state2.x = x2;
        state2.y = y2;
        state2.f = compute_distance(x2,y2, goal[0], goal[1]);
        state2.theta = theta2;
...
HBF::maze_path HBF::search(vector< vector<int> > &grid, vector<double> &start,
    ...
    int g = 0;
    int f = compute_distance(start[0],start[1],goal[0],goal[1]);
    ...
    bool finished = false;
    while(!opened.empty()) {
        sort( opened.begin( ), opened.end( ), [ ]( const auto& lhs, const auto& rhs )
        {
...
double HBF::compute_distance(double x1, double y1, double x2, double y2) {
