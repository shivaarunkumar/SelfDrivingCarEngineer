...
class DBWNode(object):
    def __init__(self):
        ...
        self.controller = Controller(vehicle_mass=vehicle_mass, wheel_radius=wheel_radius, accel_limit=accel_limit,
    ...
    def loop(self):
        rate = rospy.Rate(50)  # 50Hz : Mandatory rate limit for carla
        while not rospy.is_shutdown():
            ...
            brake = 0.
            steering = 0.
            if not None in (self.curr_vel, self.linear_vel, self.angular_vel):
                throttle, brake, steering = self.controller.control(self.is_dbw_enabled, self.curr_vel, self.linear_vel,
                                                                    self.angular_vel)
