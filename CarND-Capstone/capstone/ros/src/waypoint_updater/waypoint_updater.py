...
class WaypointUpdater(object):
    def __init__(self):
        ...
        # rospy.spin() # used to keep the node alive until user explicitly terminated it
        self.loop()

    def loop(self):
        rate = rospy.Rate(50)
        while not rospy.is_shutdown():
            if self.pose and self.base_waypoints and self.waypoints_2d and self.waypoints_tree:
    ...
    def get_closest_waypoint_idx(self):
        ...
        # Check if closest is ahead or behind the ego vehicle
        closest_coord = self.waypoints_2d[closest_idx]
        prev_coord = self.waypoints_2d[closest_idx - 1]

        # Use hyperplane to determine relative location
    ...
    def publish_waypoints(self, closest_waypoint_index):
        # lane = Lane()  # Instantiate the message to publish
